  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      <header className="border-b border-zinc-800 px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={() => router.push("/log")} className="bg-amber-500 hover:bg-amber-400 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors">+ Log Session</button>
      </header>
      <div className="flex border-b border-zinc-800">
        {(["sessions", "progression"] as const).map((v) => (
          <button key={v} onClick={() => setView(v)} className={`flex-1 py-3 text-sm font-medium capitalize transition-colors ${view === v ? "text-white border-b-2 border-amber-500" : "text-zinc-400 hover:text-white"}`}>{v}</button>
        ))}
      </div>
      <div className="flex-1 px-4 py-4 max-w-2xl mx-auto w-full">
        {view === "sessions" && (
          sessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <button onClick={() => router.push("/log")} className="bg-amber-500 hover:bg-amber-400 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-colors">Log Your First Session</button>
            </div>
          ) : (
            <div className="space-y-3">
              {sessions.map((session) => {
                ...
                return (
                  <div key={session.id} className="bg-zinc-950 rounded-xl border border-zinc-800">
                    <button onClick={() => setExpanded(isExpanded ? null : session.id)} className="w-full px-4 py-3 flex items-center justify-between text-left">
                      <div>
                        ...
                        <p className="text-xs text-zinc-400 mt-0.5">{new Date(session.date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}{session.durationMin && ` · ${session.durationMin}min`}{session.program && ` · ${session.program.name}`}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-zinc-500">{Object.keys(summary).length} exercises</span>
                      ...
                      <div className="border-t border-zinc-800 px-4 py-3 space-y-2">
                        {Object.entries(summary).map(([name, stats]) => (
                          <div key={name} className="flex items-center justify-between">
                            <div><p className="text-sm">{name}</p><p className="text-xs text-zinc-500">{stats.totalSets} sets × {stats.reps} reps</p></div>
                            <span className="text-sm font-mono font-medium">{stats.maxWeight}<span className="text-xs text-zinc-400 ml-0.5">lbs</span></span>
                          </div>
                        ))}
                        {session.notes && (
                        ...
                        <div className="pt-2 border-t border-zinc-800 flex justify-end">
                          <button onClick={() => deleteSession(session.id)} disabled={deletingId === session.id} className="text-xs text-zinc-600 hover:text-red-400 disabled:opacity-40 transition-colors">
        ...
        {view === "progression" && (
          <div className="space-y-4">
            <div>
              <label className="text-xs text-zinc-400 block mb-2">Select a lift to track</label>
              <select value={selectedExercise} onChange={(e) => setSelectedExercise(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-500">
                  ...
                  {progressionData.map((entry, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-xs text-zinc-500 w-16 shrink-0">{entry.date}</span>
                        <div className="flex-1 bg-zinc-900 rounded-full h-2"><div className="bg-amber-400 h-2 rounded-full transition-all" style={{ width: `${barWidth}%` }} /></div>
                        <span className="text-xs font-mono w-16 text-right">{entry.maxWeight}lbs</span>
                        {delta !== null && <span className={`text-xs w-10 text-right ${delta > 0 ? "text-green-400" : delta < 0 ? "text-red-400" : "text-zinc-500"}`}>{delta > 0 ? `+${delta}` : delta === 0 ? "—" : delta}</span>}
                      </div>
                    );
                  })}
                </div>
            ...
            {selectedExercise && progressionData.length === 0 && <p className="text-zinc-400 text-sm text-center py-8">No data for {selectedExercise} yet.</p>}
            {!selectedExercise && <p className="text-zinc-500 text-sm text-center py-8">Select a lift above to see your progression over time.</p>}
          </div>
        )}
      </div>
    </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="text-center"><p className="text-lg font-bold">{value}</p><p className="text-xs text-zinc-400">{label}</p></div>;
}
